...
class CLICommand:
    ...
    def __init__(self):
        ...
        """
        def _recursive(_items:CLIParamCollection, _type):
            # Check bases first
            ...
            for _varname, _value in _type.__dict__.items():
                ...
                if isinstance(_value, CLIRequiredDef):
                    _items.add_reqparam(CLIRequired(_varname, _value))
                # Is it an optional flag parameter?
                if isinstance(_value, CLIOptionFlagDef):
                    _items.add_optparam(CLIOptionFlag(_varname, _value))
                # Is it an optional parameter with an argument?
                if isinstance(_value, CLIOptionWArgDef):
                    _items.add_optparam(CLIOptionWArg(_varname, _value))
            # Finish
            return
        self.__params:CLIParamCollection = CLIParamCollection()
    ...
    def __help(self, cmdname:None|str):
        ...
        for _param in self.__params.optparams:
            if isinstance(_param, CLIOptionWArg):
    ...
    def execute(self,\
            ...
            if (not _optend) and _input.startswith('-'):
                ...
                if _input.startswith("--"):
                    ...
                    if not (_name in self.__params.optparams):
                        print(f"ERROR: Unknown option: {_name}", file = stderr)
                        return 1
                ...
                # No! Assume it's the name of a shortcut
                    ...
                    if not (_short in self.__params.shortcuts):
                        print(f"ERROR: Unknown shortcut: {_short}", file = stderr)
                        return 1
                    _opt = self.__params.shortcuts[_short]
                # Add
                if isinstance(_opt, CLIOptionWArg):
                    _optin = _opt.name
                else:
        ...
        if _optin is not None:
            print("ERROR: Input ends before optional parameter value.", file = stderr)
            return 1
        if len(reqargs) < len(self.__params.reqparams):
            print("ERROR: One or more required arguments are missing.", file = stderr)
            return 1
        # Parse arguments
        ...
        for _optarg in optargs:
            ...
            # No! This is a parameter.
                ...
                _param = cast(CLIOptionWArg, self.__params.optparams[_optarg[0]])
