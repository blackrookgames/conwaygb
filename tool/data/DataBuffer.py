class DataBuffer:
    ...
    from data.DataError import DataError as __DataError
    from helper.ErrorUtil import ErrorUtil as __ErrorUtil
    ...
    def __setitem__(self, index, value):
        try:
            ...
            self.__data[_index] = self.__ErrorUtil.valid_int(value, param = 'value')
            return
        except Exception as _e: e = _e
        raise e
    ...
    def __getindex(self, index):
        _index = self.__ErrorUtil.valid_int(index, param = 'index')
    ...
    def set_cursor(self,\
        ...
        if position < 0 or position > len(self.__data):
            if raisedata:
                if position < 0:
                    raise self.__DataError("Offset cannot be below zero.")
                raise self.__DataError("Offset cannot exceed the length of the data.")
    ...
    def read_byte(self):
        ...
        if self.__cursor == len(self.__data):
            raise self.__DataError("Unexpected end of data.")
