...
class LifePattern:
    ...
    def __init__(self,\
            width:int = 0,\
            height:int = 0,\
            rule:__LifePatternRule = __LifePatternRule()):
        """
    ...
    def __getitem__(self, key) -> numpy.bool:
        try:
    ...
    def __setitem__(self, key, value):
        try:
            index = self.__getindex(key)
            self.__data[index] = self.__ErrorUtil.valid_bool(value)
            return
        except Exception as _e:
            e = _e
        raise e
    ...
    @rule.setter
    def rule(self, value:__LifePatternRule):
        self.__rule = value
    ...
    def __setsize(self,\
        ...
        self.__data = numpy.zeros(self.__width * self.__height, dtype = bool)
