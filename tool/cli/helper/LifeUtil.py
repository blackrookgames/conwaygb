...
class LifeUtil:
    ...
    def pattern_load_rle(cls, path:str):
        ...
        def _parsehdrdef(\
                ...
                _reader:LifeUtil.__StringReader,\
                _stop:int):
            ...
            while True:
                if _reader.pos >= _stop:
                    print(_reader.error("Equals sign expected"), file = stderr)
                    return False
            ...
            if name == '':
                print(_reader.error("Name must precede equals sign"), file = stderr)
                return False
        ...
        def _parsexy(_value:str, _name:str):
            _r, _v = cls.__CLIParseUtil.to_int(_value)
            if not _r:
                return False, 0
            if _v < 0:
                print(f"ERROR: {_name} cannot be negative", file = stderr)
                return False, 0
            return True, _v
        def _parserule(_value:str, _name:str):
            def __invalid(__value):
                print(f"{__value} is not a valid pattern rule.", file = stderr)
                return False, cls.__LifePatternRule()
            if len(_value) == 0:
            ...
            if len(_s) == 0:
                return __invalid(_value)
            # Success!!!
            return True, cls.__LifePatternRule(b = tuple(_b), s = tuple(_s))
        def _parsedata(\
                _pattern:LifeUtil.__LifePattern,\
                _reader:LifeUtil.__StringReader):
            def __inc(\
                    __reader:LifeUtil.__StringReader,\
                    __i:int,\
                    __size:int,\
                    __count:int):
                __i += __count
                if __i > __size:
                    print(__reader.error("Exceeded cell count"), file = stderr)
                    return -1
                return __i
            def __writecell(
                    __pattern:LifeUtil.__LifePattern,\
                    __i:int,\
                    __value:bool,\
                    __reader:LifeUtil.__StringReader,\
                    __size:int):
            ...
            if _size > 0:
                while True:
                    # End of file (FAIL)?
                    if _reader.eof:
                        print(_reader.error_unex_end(), file = stderr)
                        return False
                    # Comment?
                    elif _reader.chr == '#':
                        _reader.skip_line()
                ...
                if _reader.chr != '!':
                    print(_reader.error("Exceeded cell count"), file = stderr)
                    return False
            # Success!!!
            return True
        # Load string
        ...
        if string is None: return None
        # Begin parsing
        reader = cls.__StringReader(string)
        # Find header
        while True:
            # End of file?
            if reader.eof:
                print(reader.error_unex_end(), file = stderr)
                return None
            # Whitespace?
            if reader.white:
                reader.next()
            ...
            break
        # Parse header
        _hdrreader = cls.__StringReader(reader)
    ...
    @classmethod
    def pattern_save_rle(cls, pattern:__LifePattern, path:str):
        """
